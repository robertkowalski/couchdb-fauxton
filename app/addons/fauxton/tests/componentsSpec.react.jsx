...
define([
  ...
  describe('Tray', function () {
    ...
    beforeEach(function () {
      ...
      trayEl = TestUtils.renderIntoDocument(React.createElement(Views.Tray, {className: "traytest", trayid: "testtray"}), container);
    });
    ...
    it('registers handler with body', function () {
      ...
      assert(handlersOn[0][1].substring(0, 6) === "click.");
      assert(handlersOn[0][1].indexOf("testtray") >= 0);
  ...
  describe('Pagination', function () {
    ...
    it("renders 20-wise pages per default", function () {
      var pageEl = TestUtils.renderIntoDocument(React.createElement(Views.Pagination, {page: 3, total: 55, urlPrefix: "?prefix=", urlSuffix: "&suffix=88"}), container);
    ...
    it("can overwrite collection size", function () {
      var pageEl = TestUtils.renderIntoDocument(React.createElement(Views.Pagination, {perPage: 10, page: 3, total: 55}), container);
      var lis = pageEl.getDOMNode().getElementsByTagName("li");
      assert.equal(1 + 6 + 1, lis.length);
    });

    it("handles large collections properly - beginning", function () {
      var pageEl = TestUtils.renderIntoDocument(React.createElement(Views.Pagination, {page: 3, total: 600}), container);
      var lis = pageEl.getDOMNode().getElementsByTagName("li");
    ...
    it("handles large collections properly - middle", function () {
      var pageEl = TestUtils.renderIntoDocument(React.createElement(Views.Pagination, {page: 10, total: 600}), container);
      var lis = pageEl.getDOMNode().getElementsByTagName("li");
      assert.equal(1 + 10 + 1, lis.length);
    ...
    it("handles large collections properly - end", function () {
      var pageEl = TestUtils.renderIntoDocument(React.createElement(Views.Pagination, {page: 29, total: 600}), container);
      var lis = pageEl.getDOMNode().getElementsByTagName("li");
      assert.equal(1 + 10 + 1, lis.length);
